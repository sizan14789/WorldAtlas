...
const Countries = () => {
  ...
  const filteredData = ()=>{
    const filtered = (countries as countryInterface[]).filter((cur) =>
      cur.name.common.toLowerCase().includes(search.toLowerCase()) // Case-insensitive match
    );
    return filtered
  }
      ...
      <main className="my-auto">
        <div className="secs flex flex-col">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {(filteredData() as countryInterface[]).slice(0, 20).map((each, index) => {
